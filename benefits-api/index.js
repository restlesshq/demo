...
const restless = require("@restlessai/sdk")(
  "rstlss_1bfa67b746f33c71cd640433214ea0d0a6a4a92a66d01979f8bb87a3373ef306",
);
...
const sendErr = (res, err) => {
  if (err instanceof ApiError) {
    res
      .status(err.status)
      .set("x-restless-error-code", err.code)
  ...
  res
    .status(500)
    .set("x-restless-error-code", "internal_error")
    .json({
...
app.use((req, res, next) => {
  ...
  const token = auth.startsWith("Token ")
    ? auth.slice(6)
...
app.use(
  restless.setup((req) => {
    ...
    return {
      apiKey: restless.mask(token || undefined),
      owner: req.account
        ? {
            id: req.account.companyId || req.account.id,
            label: req.account.company || req.account.name,
...
const findCompany = (req) =>
  companies.find((c) => c.id === req.account.companyId);
...
app.get(
  ...
  wrap((req, res) => {
    const emp = employees.find(
      (e) => e.id === req.params.id && e.companyId === req.account.companyId,
    );
    if (!emp)
      throw new ApiError(
        "employee_not_found",
        `No employee with id ${req.params.id}`,
        404,
      );
    res.json(emp);
  }),
...
app.post(
  ...
  wrap((req, res) => {
    ...
    if (
      req.body.employmentType === "salary" &&
      req.body.annualSalaryCents == null
    ) {
      throw new ApiError(
        "invalid_request",
        "annualSalaryCents required for salaried employees.",
        422,
      );
    }
    if (
      req.body.employmentType === "hourly" &&
      req.body.hourlyRateCents == null
    ) {
      throw new ApiError(
        "invalid_request",
        "hourlyRateCents required for hourly employees.",
        422,
      );
    }
    const emp = {
...
app.post(
  ...
  wrap((req, res) => {
    ...
    emp.terminationDate =
      req.body?.terminationDate ?? new Date().toISOString().slice(0, 10);
...
app.get(
  ...
  wrap((req, res) => {
    const p = payrolls.find(
      (x) => x.id === req.params.id && x.companyId === req.account.companyId,
    );
    if (!p)
      throw new ApiError(
        "payroll_not_found",
        `No payroll with id ${req.params.id}`,
        404,
      );
    res.json(p);
  }),
...
app.post(
  ...
  wrap((req, res) => {
    ...
    const p = {
      ...
      companyId: req.account.companyId,
      status: "draft",
      employees: scoped(req, employees)
...
app.get(
  ...
  wrap((req, res) => {
    ...
    if (!b)
      throw new ApiError(
        "benefit_not_found",
        `No benefit with id ${req.params.id}`,
        404,
      );
    res.json(b);
  }),
...
app.post(
  ...
  wrap((req, res) => {
    ...
    const emp = employees.find(
      (e) =>
        e.id === req.body.employeeId && e.companyId === req.account.companyId,
    );
    if (!emp)
      throw new ApiError(
        "employee_not_found",
        `No employee with id ${req.body.employeeId}`,
        422,
      );
    if (!benefits.find((b) => b.id === req.body.benefitId)) {
      throw new ApiError(
        "benefit_not_found",
        `No benefit with id ${req.body.benefitId}`,
        422,
      );
    }
    const enr = {
  ...
  wrap((req, res) => {
    ...
    if (idx === -1)
      throw new ApiError(
        ...
        `No enrollment with id ${req.params.id}`,
        404,
      );
...
app.get(
  ...
  wrap((req, res) => {
    const emp = employees.find(
      (e) =>
        e.id === req.params.employeeId && e.companyId === req.account.companyId,
    );
    if (!emp)
      throw new ApiError(
        "employee_not_found",
        `No employee with id ${req.params.employeeId}`,
        404,
      );
    const list = timeOffBalances.filter(
      (b) => b.employeeId === req.params.employeeId,
    );
...
app.post(
  ...
  wrap((req, res) => {
    ...
    if (!r)
      throw new ApiError(
        "request_not_found",
        `No request with id ${req.params.id}`,
        404,
      );
    if (r.status !== "pending") {
      throw new ApiError(
        "request_not_pending",
        `Request is ${r.status} and cannot be approved.`,
        409,
      );
    }
    r.status = "approved";
...
app.post(
  ...
  wrap((req, res) => {
    ...
    if (r.status !== "pending") {
      throw new ApiError(
        "request_not_pending",
        `Request is ${r.status} and cannot be denied.`,
        409,
      );
    }
    r.status = "denied";
...
app.get(
  ...
  wrap((req, res) => {
    ...
    if (!d)
      throw new ApiError(
        "document_not_found",
        `No document with id ${req.params.id}`,
        404,
      );
    res.json(d);
  }),
...
app.use((req, res) => {
  sendErr(
    res,
    new ApiError("not_found", `No route for ${req.method} ${req.path}`, 404),
  );
});
